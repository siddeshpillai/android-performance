...
public class CachingActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        theButtonThatDoesFibonacciStuff.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Map<Integer, Integer> memoizationMap = new HashMap<Integer, Integer>();
                Log.i(LOG_TAG, String.valueOf(computeFibonacci(40, memoizationMap)));
            }
        });
     ...
     */
    public int computeFibonacci(int positionInFibSequence, Map<Integer, Integer> map) {
        if (positionInFibSequence == 0) {
            return 0;
        }
        if (positionInFibSequence == 1) {
            return 1;
        }

        if (map.containsKey(positionInFibSequence)) {
            return map.get(positionInFibSequence);
        }

        Integer fibForN = computeFibonacci(positionInFibSequence - 1, map) + computeFibonacci(positionInFibSequence -2, map);
        map.put(positionInFibSequence, fibForN);
        return fibForN;
    }
}
